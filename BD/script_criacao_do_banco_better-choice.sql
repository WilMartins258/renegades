...
CREATE PROCEDURE VerificarLatitudeNula()
BEGIN
    DECLARE estabelecimento_id INT;
    DECLARE done INT DEFAULT FALSE;
    DECLARE cur CURSOR FOR SELECT id FROM estabelecimento WHERE latitude IS NULL;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

    -- Cria uma tabela temporária para armazenar os resultados
    CREATE TEMPORARY TABLE IF NOT EXISTS estabelecimentos_sem_latitude (id INT);

    -- Abre o cursor
    OPEN cur;

    -- Loop para obter os IDs e inserir na tabela temporária
    read_loop: LOOP
        FETCH cur INTO estabelecimento_id;
        IF done THEN
            LEAVE read_loop;
        END IF;
        INSERT INTO estabelecimentos_sem_latitude (id) VALUES (estabelecimento_id);
    END LOOP;
    ...
    SELECT * FROM estabelecimentos_sem_latitude;

    -- Remove a tabela temporária (opcional, dependendo do seu caso de uso)
    DROP TEMPORARY TABLE IF EXISTS estabelecimentos_sem_latitude;
END;
//
